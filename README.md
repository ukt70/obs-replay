# Настройка обс для откатов как shadowplay с отдельным профилем для записи/стримов
...
7. для автозапуска от админа [https://obsproject.com/forum/threads/start-obs-as-administrator-on-startup-in-windows-10-with-startreplaybuffer.116313/]:
   ...
   3) Общие: называем OBS Studio autostart, ставим галку "Выполнить с наивысшими правами"
      ...
      Действия: 
      1. (для выключения ошибки о неправильном закрытии обс) создать, Действие Запуск программы, Программа или сценарий: cmd.exe, Добавить аргументы: /c "rd /s /q %APPDATA%\obs-studio\.sentinel"
      2. создать, Действие Запуск программы, Программа или сценарий: указываем путь к obs64.exe ("C:\Program Files\obs-studio\bin\64bit\obs64.exe"), Добавить аргументы: --startreplaybuffer --minimize-to-tray , Рабочая папка: папка с obs64.exe (C:\Program Files\obs-studio\bin\64bit)
